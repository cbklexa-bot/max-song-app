...
#topup-modal .amount{min-height:56px;border-radius:15px}
#topup-modal .amount-left strong{font-size:11px}
#topup-modal .amount-left small{font-size:8px}
#topup-modal .bonus{color:#d9b9ff;background:rgba(145,72,255,.09);border-color:rgba(174,100,255,.14)}
...
(function(){
if(window.__AI_HOME_ACCOUNT_PATCH__)return;window.__AI_HOME_ACCOUNT_PATCH__=true;
function textOf(selector,fallback){const el=document.querySelector(selector);const value=el?.textContent?.trim();return value||fallback}
function syncAccount(){const name=textOf('#user-name','Пользователь MAX'),balance=textOf('#balance-value','0 ₽'),nameEl=document.getElementById('ai-home-account-name'),balanceEl=document.getElementById('ai-home-account-balance-value'),avatarEl=document.getElementById('ai-home-account-avatar');if(nameEl)nameEl.textContent=name;if(balanceEl)balanceEl.textContent=balance;if(avatarEl)avatarEl.textContent=(name.charAt(0)||'П').toUpperCase()}
function syncTopupPlans(){
 const buttons=Array.from(document.querySelectorAll('#topup-modal .amount')).slice(0,3);if(buttons.length<3)return;
 const plans=[
  {amount:400,bonus:20,percent:5,total:420,info:'Бонус +20 ₽'},
  {amount:800,bonus:80,percent:10,total:880,info:'Бонус +80 ₽'},
  {amount:2400,bonus:480,percent:20,total:2880,info:'Бонус +480 ₽'}
 ];
 buttons.forEach((button,index)=>{const plan=plans[index];button.dataset.amount=String(plan.amount);button.innerHTML='<span class="amount-left"><strong>'+plan.amount+' ₽ <span class="bonus">+'+plan.percent+'%</span></strong><small>'+plan.info+'</small></span><span class="amount-right">'+plan.total+' ₽</span>';});
 if(!document.querySelector('#topup-modal .amount.selected'))buttons[0].classList.add('selected');
 const selected=document.querySelector('#topup-modal .amount.selected');const plan=plans[buttons.indexOf(selected)];if(plan)document.getElementById('credit-value').textContent=plan.total+' ₽';
 buttons.forEach((button,index)=>{if(button.dataset.homePlanBound==='1')return;button.dataset.homePlanBound='1';button.addEventListener('click',function(){const plan=plans[index];if(!plan)return;buttons.forEach(item=>item.classList.remove('selected'));button.classList.add('selected');document.getElementById('credit-value').textContent=plan.total+' ₽';});});
}
function openTopup(){try{if(typeof window.openTopup==='function'){window.openTopup();return}document.getElementById('balance-button')?.click()}catch(error){console.warn('[AI HOME ACCOUNT] topup',error)}}
function ensureAccount(){const home=document.getElementById('ai-gifts-home'),mount=document.getElementById('ai-home-account');if(!home||!mount)return false;if(!document.querySelector('#ai-home-account .ai-home-account-profile'))mount.innerHTML='<div class="ai-home-account"><div class="ai-home-account-profile"><div id="ai-home-account-avatar" class="ai-home-account-avatar">П</div><div class="ai-home-account-info"><p class="ai-home-account-label">Ваш MAX аккаунт</p><p id="ai-home-account-name" class="ai-home-account-name">Загрузка...</p><p class="ai-home-account-subtitle">Имя берётся из вашего профиля MAX</p></div></div><button id="ai-home-balance-button" class="ai-home-account-balance" type="button" aria-label="Пополнить баланс"><div><span class="ai-home-account-balance-label">Баланс</span><span id="ai-home-account-balance-value" class="ai-home-account-balance-value">0 ₽</span></div><span class="ai-home-account-plus">+</span></button></div>';
const button=document.getElementById('ai-home-balance-button');if(button&&!button.dataset.bound){button.dataset.bound='1';button.addEventListener('click',openTopup)}syncAccount();syncTopupPlans();const sourceName=document.getElementById('user-name'),sourceBalance=document.getElementById('balance-value');if(sourceName&&!sourceName.dataset.homeAccountObserved){sourceName.dataset.homeAccountObserved='1';new MutationObserver(syncAccount).observe(sourceName,{subtree:true,childList:true,characterData:true})}if(sourceBalance&&!sourceBalance.dataset.homeAccountObserved){sourceBalance.dataset.homeAccountObserved='1';new MutationObserver(syncAccount).observe(sourceBalance,{subtree:true,childList:true,characterData:true})}return true}
function ensureTransition(){if(document.getElementById('ai-song-page-transition'))return;const overlay=document.createElement('div');overlay.id='ai-song-page-transition';overlay.innerHTML='<div class="transition-bg"></div><div class="transition-orb"></div><div class="transition-ring"></div><div class="transition-pulse"></div><div class="transition-content"><p class="transition-kicker">Персональная музыка</p><h2 class="transition-label">Песня под заказ</h2><div class="transition-line"></div></div>';document.body.appendChild(overlay)}
function startTransition(){const button=document.getElementById('ai-home-song');if(!button||button.dataset.transitionBound==='1')return;button.dataset.transitionBound='1';button.addEventListener('click',function(event){event.preventDefault();event.stopPropagation();ensureTransition();const overlay=document.getElementById('ai-song-page-transition');if(!overlay||window.__AI_HOME_TRANSITION_BUSY__)return;window.__AI_HOME_TRANSITION_BUSY__=true;overlay.classList.remove('active');void overlay.offsetWidth;overlay.classList.add('active');setTimeout(function(){try{if(typeof window.__AI_HOME_PREPARE_SONG__==='function')window.__AI_HOME_PREPARE_SONG__();else{document.getElementById('ai-gifts-home').style.display='none';document.querySelector('.app').style.display='block';window.__AI_SONG_SCREEN__=true}}finally{setTimeout(function(){overlay.classList.remove('active');window.__AI_HOME_TRANSITION_BUSY__=false},180)}},2450)})}
function boot(){ensureAccount();startTransition();setTimeout(ensureAccount,350);setTimeout(ensureAccount,900);setTimeout(startTransition,350)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
})();</script>`;
function inject(body){if(typeof body!=='string'||!body.includes('<body'))return body;if(body.includes('ai-home-account-script'))return body;return body.replace(/<\/body>/i,extraCss+'\n'+extraScript+'\n</body>')}
express.response.sendFile=function patchedSendFile(filePath,...args){const isIndex=typeof filePath==='string'&&/(?:^|[\\/])index\.html$/i.test(filePath);if(!isIndex)return originalSendFile.call(this,filePath,...args);const response=this,originalSend=response.send;response.send=function aiHomeAccountSend(body){try{return originalSend.call(this,inject(body))}catch(error){console.error('[AI HOME ACCOUNT]',error.message);return originalSend.call(this,body)}};try{return originalSendFile.call(this,filePath,...args)}finally{response.send=originalSend}};
