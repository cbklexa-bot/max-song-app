...
function buildPaymentUrl(plan, invoiceId, email) {
...
function install(app) {
  ...
  app.post('/api/robokassa/live-sbp', async (req, res) => {
    try {
      ...
      const paymentUrl = buildPaymentUrl(plan, id, email);
      ...
      return res.json({ ok: true, paymentUrl, invoiceId: id, amount: plan.amount, creditedAmount: plan.credited });
    } catch (e) {
      console.error('[LIVE SBP]', e.response?.data || e.message);
      return res.status(e.response?.status === 401 || e.response?.status === 403 ? 503 : 400).json({ ok: false, error: e.message || 'Не удалось начать оплату СБП' });
    }
  });
}
