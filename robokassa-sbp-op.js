...
function validateMax(initData) {
  ...
  const params = {};
  for (const part of String(initData).split('&')) { const i = part.indexOf('='); if (i >= 0) params[part.slice(0, i)] = part.slice(i + 1); }
...
function install(app) {
  ...
  app.post('/api/robokassa/start-sbp', async (req, res) => {
    try {
      ...
      await dbPost('payments', {
        ...
        confirmation_url: null, metadata: { provider: 'robokassa', payment_method: 'SBP', invoice_id: invoiceId, email }
      });
      console.log('[ROBOKASSA SBP OP]', { invoiceId, amount: plan.amount, user: String(user.id) });
      return res.json({ ok: true, merchantLogin: LOGIN, invoiceId, outSum, signature, amount: plan.amount, bonus: plan.bonus, creditedAmount: plan.credited });
    } catch (error) {
  ...
  // Keep both callback paths during rollout so no existing Robokassa setting breaks.
