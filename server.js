...
const RENDER_BACKEND = 'https://max-song-app.onrender.com';
const isRender = process.env.RENDER || process.env.RENDER_SERVICE_ID;

// Если мы на Amvera (не на Render), включаем режим прокси-моста к Render
if (!isRender) {
    console.log('🔄 Running in PROXY mode to Render:', RENDER_BACKEND);
    ...
    app.use('/api', async (req, res) => {
        try {
            ...
            res.status(500).json({ success: false, message: 'Proxy error to Render: ' + err.message });
        }
    });
} else {
    // --- ТВОЙ ОРИГИНАЛЬНЫЙ КОД ДЛЯ RENDER ---
