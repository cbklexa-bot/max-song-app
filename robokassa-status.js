...
async function getInvoiceInformation(invId) {
  const jwt = makeJwt({ MerchantLogin: ROBOKASSA_MERCHANT_LOGIN, InvId: Number(invId) });
  const response = await axios.post(
  ...
  app.get('/api/robokassa/status', async (req, res) => {
    try {
      ...
      if (payment.status !== 'pending') {
        return res.json({ ok: true, status: payment.status, invoiceId });
      }
      ...
      if (!Number.isFinite(providerAmount) || providerAmount !== localAmount) {
        return res.status(409).json({ ok: false, error: 'Payment amount mismatch' });
      }

      if (info?.invoiceStatus === 'Paid') {
        const processed = await supabaseRpc('process_robokassa_payment', {
      ...
      return res.json({
        ok: true,
        status: info?.invoiceStatus === 'Expired' ? 'expired' : 'pending',
        invoiceId
      });
    } catch (error) {
